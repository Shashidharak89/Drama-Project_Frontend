...
export default function ActorForm() {
  ...
  async function handleSubmit(e) {
    ...
    const body = { name, plays: parse(playsText) };
    ...
    try {
      ...
      setStatus({ ok: true, msg: 'Actor inserted', data });
      setName('');
      setPlaysText('');
    } catch (err) {
  ...
  return (
    <form className="tf-form" onSubmit={handleSubmit}>
      <h3 className="tf-form-title">Insert Actor</h3>
      ...
      <div className="tf-actions">
        <button className="tf-button" type="submit" disabled={loading}>
          {loading ? 'Inserting...' : 'Insert Actor'}
        </button>
      </div>
